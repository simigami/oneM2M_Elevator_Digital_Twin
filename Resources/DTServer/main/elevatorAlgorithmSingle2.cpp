...
void elevatorAlgorithmSingle::run(socket_oneM2M* sock, socket_UnrealEngine* ueSock, physics* phy)
{
	...
	try
	{
		while (thisElevatorFlag->isRunning)
		{
			...
			if (thisElevatorFlag->isRunning == true && !thist->thisElevatorStatus->current_goTo_floor_vector_info->empty())
			{
	...
	{
		std::wcout << "IN " << building_name << " -> " << device_name << " ERROR OCCURR AT : elevatorAlgorithmSingle::run " << e.what() << std::endl;
	}
}
...
void elevatorAlgorithmSingle::updateElevatorTick(socket_UnrealEngine* ueSock, physics* phy)
{
	...
	try
	{
		if (temp->button_inside.empty() && temp->button_outside_floor == 5)
		{
